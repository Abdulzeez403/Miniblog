...
const Bloglist = ({ blogs, title }) => {
  return (
    <div className="container">
      ...
      {blogs.map((blog) => {
        return (
          <div className="container shadow" key={blog.id}>
            <Link
              className="row  row -d-sm-inline  text-decoration-none text text-dark d-flex justify-content-between py-3 my-4 mx-1"
              to={`/blogs/${blog._id}`}
            >
